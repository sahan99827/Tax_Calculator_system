...
function calculatePayableTax(salary) {
  ...
  for (const slab of MONTHLY_TAX_SLABS) {
    ...
    if (incomeInSlab > 0) {
      ...
      if (slabRate > 0) {
        taxBreakdown.push({
          range: `${formatter.format(previousMax + 1)} - ${formatter.format(
            slabLimit
          )}`,
...
const contentData = {
  ...
  withholding: () =>
    `<h2><i class="fas fa-hand-holding-usd me-2"></i> Withholding Tax</h2><p>This module handles Rent Tax, Bank Interest Tax, and Dividend Tax calculations.</p>`,
  income: () =>
    `<h2><i class="fas fa-calendar-alt me-2"></i> Income Tax (Annual)</h2><p>This module calculates annual progressive tax based on annual income slabs.</p>`,
  sscl: () =>
    `<h2><i class="fas fa-file-invoice-dollar me-2"></i> SSCL Tax</h2><p>This module handles Sale Tax (2.5%) and VAT (15%) calculations.</p>`,
  leasing: () =>
    `<h2><i class="fas fa-car me-2"></i> Leasing Calculations</h2><p>This module calculates Monthly Installments and Max Loan Value using the EMI formula $(A\times i)/(1-(1/(1+i)^{\wedge}n))$[cite: 86].</p>`,
};
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (isNaN(salary) || salary <= 0) {
      ...
      feedbackDiv.textContent =
        "Please enter a valid salary amount (must be a number > 0).";
    ...
    // Display results
    document.getElementById("taxAmountResult").textContent = formatter.format(
      result.taxAmount
    );
    document.getElementById("netSalaryResult").textContent = formatter.format(
      result.netSalary
    );
    document.getElementById("appliedRatesResult").textContent =
      result.appliedRates;
...
navLinks.forEach((link) => {
  link.addEventListener("click", function (e) {
    ...
    if (section === "clear") {
      ...
      Swal.fire({
        title: "Functionality to clear all forms in other modules will be added here.",
        icon: "success",
        draggable: true,
      });
      return;
